...
INSERT INTO `friends` VALUES (1,'emi','an','a'),(10,'juan','emi','a'),(11,'juan','maria','p'),(12,'emi','carlos','p');
...
CREATE TABLE `users` (
  ...
  `connectedIP` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`),
...
INSERT INTO `users` VALUES (1,'emi','123','C',1),(15,'carla','123','D',NULL),(14,'an','123','D',NULL),(13,'carlos','123','C',2),(12,'juan','123','D',NULL),(16,'maria','123','D',NULL);
...
IF exists (SELECT * FROM final.friends WHERE user1 = $u and user2 = $p) then
		set aux = 'Invitation already sent';
        select aux;
elseiF exists (SELECT * FROM final.friends WHERE user2 = $u and user1 = $p) then
		set aux = 'Invitation already received';
        select aux;
elseif exists (SELECT * from final.users where user = $p and $u != $p) then
...
BEGIN
SELECT * FROM final.friends WHERE (user1 = $u) or 
(user2 = $u);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
elseiF exists (SELECT * FROM final.friends WHERE user1 = $u and user2 = $p) then
	...
	set aux = 'No invitation received';
    select aux;
end if;
END ;;
DELIMITER ;
...
declare aux char(30) default "";
IF exists (SELECT * FROM final.users WHERE user = $u) then
		set aux = 'Name already taken';
        select aux;
else
	INSERT INTO `final`.`users` (`user`, `pass`) VALUES ($u, $p);
	set aux = 'User registered';
end if;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SetConnectionState`(IN $u CHAR(20), IN $s char(20), IN $ip int)
BEGIN
iF exists (SELECT * FROM final.users WHERE user = $u) then
		UPDATE `final`.`users` SET `ConnectedState`= $s WHERE `user`= $u;
		UPDATE `final`.`users` SET `connectedIP`= $ip WHERE `user`= $u;
end if;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2019-02-27 19:27:43
